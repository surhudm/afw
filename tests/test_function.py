...
def referenceChebyshev1Polynomial2(x, y, params):
    ...
    for ii in range(0, len(params)):
        ...
        retVal += params[ii] * \
            referenceChebyshev1(x, x_order) * referenceChebyshev1(y, y_order)
        if x_order > 0:
            y_order += 1
        else:
            order += 1
...
class FunctionTestCase(lsst.utils.tests.TestCase):
            ...
            for y in np.arange(yMin, yMax + yDelta/2.0, yDelta):
                ...
                for x in np.arange(xMin, xMax + xDelta/2.0, xDelta):
                    ...
                    if maxXNorm is None or xNorm > maxXNorm:
                        ...
                        self.assertFloatsAlmostEqual(
                            h(x, y), predVal, msg=msg, atol=self.atol, rtol=None)

                msg = self.normErr.format("x", xMin, xMax, minXNorm, maxXNorm)
                self.assertFloatsAlmostEqual(
                    minXNorm, -1., msg=msg, atol=self.atol)
                self.assertFloatsAlmostEqual(
                    maxXNorm, 1., msg=msg, atol=self.atol)

            msg = self.normErr.format("y", yMin, yMax, minYNorm, maxYNorm)
            self.assertFloatsAlmostEqual(
                minYNorm, -1., msg=msg, atol=self.atol)
    ...
    def testChebyshev1Function2DTruncate(self):
        ...
        for order in range(maxOrder + 1):
            ...
            for truncOrder in range(order + 1):
                ...
                g = afwMath.Chebyshev1Function2D(
                    fullParams[0:truncNParams], xyRange)
                ...
                for x in np.arange(xMin, xMax + xDelta/2.0, xDelta):
                    ...
                    msg = self.normErr.format(
                        "y", yMin, yMax, minYNorm, maxYNorm)
                    self.assertFloatsAlmostEqual(
                        minYNorm, -1.0, msg=msg, atol=self.atol, rtol=None)
                    self.assertFloatsAlmostEqual(
                        maxYNorm, 1.0, msg=msg, atol=self.atol, rtol=None)

                msg = self.normErr.format("x", xMin, xMax, minXNorm, maxXNorm)
                self.assertFloatsAlmostEqual(
                    minXNorm, -1.0, msg=msg, atol=self.atol, rtol=None)
                self.assertFloatsAlmostEqual(
        ...
        for sigma1 in (0.1, 1.0, 3.0):
            for sigma2 in (0.1, 1.0, 3.0):
                for angle in (0.0, 0.4, 1.1):
                    ...
                    msg = areaMsg % (type(f).__name__,
                                     approxArea, sigma1, sigma2)
            ...
            for yo in np.arange(-5.0, 5.0, 1.0):
                ...
                for x in np.arange(-5.0, 5.0, 1.0):
                    for y in np.arange(-5.0, 5.0, 1.0):
                        ...
                        msg = errMsg.format(type(f).__name__,
                                            f(x, y), predVal, x, y, xo, yo)
                        self.assertFloatsAlmostEqual(
                            f(x, y), predVal, msg=msg, atol=self.atol, rtol=None)
                        msg = errMsg.format(type(g).__name__,
                                            g(x, y), predVal, x, y, xo, yo) + "; clone"
                        self.assertFloatsAlmostEqual(
                            g(x, y), predVal, msg=msg, atol=self.atol, rtol=None)
        ...
        for n in range(1, 5):
            ...
            for xOffset in (-10.0, 0.0, 0.05):
                ...
                for x in np.arange(-10.0, 10.1, 0.50):
                    ...
                    msg = errMsg.format(type(f).__name__,
                                        f(x), predVal, n, x, xOffset, xAdj)
                    self.assertFloatsAlmostEqual(
                        f(x), predVal, msg=msg, atol=self.atol, rtol=None)
                    msg = errMsg.format(type(g).__name__,
                                        g(x), predVal, n, x, xOffset, xAdj) + "; clone"
                    self.assertFloatsAlmostEqual(
                        g(x), predVal, msg=msg, atol=self.atol, rtol=None)
    ...
    def testPolynomialFunction2D(self):
        ...
        for order, numParams in enumerate(numParamsList):
            ...
            for y in np.arange(-10.0, 10.1, 2.5):
                for x in np.arange(-10.0, 10.1, 2.5):
                    ...
                    self.assertFloatsAlmostEqual(
                        f(x, y), predVal, msg=msg, atol=2e-12, rtol=None)
                    msg = errMsg.format(type(g).__name__,
                                        g(x, y), predVal, x, y, params,
                                        "order constructor")
                    self.assertFloatsAlmostEqual(
                        g(x, y), predVal, msg=msg, atol=2e-12, rtol=None)
                    msg = errMsg.format(type(h).__name__,
                                        h(x, y), predVal, x, y, params, "clone")
                    self.assertFloatsAlmostEqual(
                        h(x, y), predVal, msg=msg, atol=2e-12, rtol=None)
        ...
        for addValue in (0.0, 0.2):
            ...
            f.setParameters(params)
            predVal = basic2DPoly(x, y, params)
            msg = errMsg.format(type(f).__name__, f(x, y),
                                predVal, x, y, params, "")
            self.assertFloatsAlmostEqual(
                f(x, y), predVal, msg=msg, atol=self.atol, rtol=None)
